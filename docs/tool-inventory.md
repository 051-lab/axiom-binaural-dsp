...
| `scripts/validate_axiom_r012_candidate.py` | Normalized approved-diff gate allowing only the owner-approved R011-to-R012 naming, width-default, interpolation-state, and saturation-arithmetic changes. | No | No | Console only. |
...
| `scripts/start_wsl_jdsp_listening.sh` | Start the managed Windows/WSL JamesDSP listening route, expose the private `JamesDSP` Pulse sink, and hot-reload the accepted Axiom Liveprog script. | Yes | No | Starts local background audio/UI processes and writes JDSP config/preset state outside repo. |
| `scripts/play_wsl_jdsp_audio.sh` | Decode a local audio file with `ffmpeg` and stream it through the managed `JamesDSP` sink for informal Axiom/JDSP listening. | Yes | No | Plays local audio only; source files and listening notes stay outside repo unless sanitized. |
| `scripts/axiom_music_library.sh` | Browse and play files from the local Windows music library through the managed WSL/JDSP listening route. | Indirect | No | Reads local music filenames and plays selected local audio; private library paths stay local. |
| `scripts/stop_wsl_jdsp_listening.sh` | Stop the managed Windows/WSL JamesDSP listening route and attempt to restore the normal WSLg PulseAudio route. | Yes | No | Stops local background audio/UI processes outside repo. |
| `scripts/start_axiom_player.sh` | Start the local browser Axiom Player, backed by the managed WSL/JDSP route, `mpv`, and the embedded JamesDSP noVNC UI. | Yes | No | Starts local app/audio/UI processes; playback state stays under `~/.local/state/axiom-engineering/player/`. |
...
| `tools/axiom-codex/axiom_codex.py` | Safe Codex-side orchestration helpers for status, local review snapshots, task-state validation, next-action planning, ready checks, command surface discovery, Agentic contract audit, guard preflights, role profiles, validated multi-role review creation/status with optional subagent provenance, Knowledge search/source audits, canonical Airwindows metadata indexing/auditing, Pi handoff briefs, and skill behavior eval fixtures. | No | No | Console output only unless report output flags are provided; completed reviews and subagent findings remain non-authoritative planning artifacts; private Knowledge paths are hidden by default; Airwindows indexes stay local and metadata-only; guard findings block unsafe publication scope but do not approve release. |
