...
`scripts/run_jdsp_accepted_stress.py` characterizes accepted `.8` at the tracked `-1.00 dB` host threshold before another DSP candidate is justified. It repeats each registered external excerpt, rejects clipped or unusable level profiles, and records stable peaks above `-0.50 dBFS` as existing limiter-pressure behavior rather than retroactively failing the accepted listening baseline. This artifact is the dense-material host reference for deciding whether a later candidate worsens limiter interaction.

`scripts/run_jdsp_sub_slider_map.py` expands that measurement across temporary accepted-`.8` fixtures whose only difference is the `Sub Harmonics Gain` default. Its default grid of `+4`, `+6`, `+8`, `+10`, and `+12 dB` establishes whether real program material remains unclipped as user bass gain rises. A failure at the accepted `+4 dB` default rejects the baseline measurement; clipping found only at elevated settings records an operating-range boundary and supplies evidence for deciding whether a later bass-reserve revision is worthwhile. Because `.8` adds global output reserve above `+4 dB`, the map also flags repeatable RMS reduction exceeding `1 dB` relative to default; that is a documented loudness-versus-headroom tradeoff rather than clipping.
